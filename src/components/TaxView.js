...
const PORTF = [
  { key: 'etoro', owner: 'José',  broker: 'eToro', tax: 'irpf', regime: 'IRPF · base del ahorro', color: 'var(--etoro)' },
  { key: 'xtb',   owner: 'Ana',   broker: 'XTB',   tax: 'irpf', regime: 'IRPF · base del ahorro', color: 'var(--xtb)' },
  { key: 'ibkr',  owner: 'Index', broker: 'IBKR',  tax: 'is',   regime: 'Impuesto de Sociedades', color: 'var(--ibkr)' },
]

const IS_MODES = {
  micro:   { label: 'Microempresa (<1M€)',     hint: '19% hasta 50k · 21% resto' },
  erd:     { label: 'Reducida dimensión (1-10M€)', hint: '23% plano' },
  general: { label: 'General',                 hint: '25% plano' },
  nueva:   { label: 'Nueva creación',          hint: '15% plano' },
}

// IRPF base del ahorro 2026 (tramos progresivos estatales)
...
// Impuesto de Sociedades 2026 según régimen
function isTax(base, mode) {
  if (base <= 0) return 0
  if (mode === 'general') return base * 0.25
  if (mode === 'erd')     return base * 0.23
  if (mode === 'nueva')   return base * 0.15
  const a = Math.min(base, 50000), b = Math.max(0, base - 50000) // micro
  return a * 0.19 + b * 0.21
}

function computeTax(p, gains, div, includeDiv, isMode) {
  const base = (gains || 0) + (includeDiv ? (div || 0) : 0)
  const tax = p.tax === 'irpf' ? irpfAhorro(base) : isTax(base, isMode)
...
export default function TaxView({ taxData, onRefresh }) {
  ...
  useEffect(() => {
    ...
    if (!years[currentYear]) years[currentYear] = empty()
    setModel({
      is_mode: base.is_mode || 'micro',
  ...
  const setMode = (m) => persist({ ...model, is_mode: m })
  ...
  const yearTotals = (y) => PORTF.reduce((s, p) => {
    ...
    return s + computeTax(p, cell.gains, cell.div, inc, model.is_mode).tax
  }, 0)
    ...
    <div className="bg-white rounded-xl border border-slate-200">
      <div className="px-5 pt-5 pb-3 flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-1.5">
            <span className="text-[11px] text-slate-400">Index (IS):</span>
            <select value={model.is_mode} onChange={e => setMode(e.target.value)}
              className="text-[11px] border border-slate-200 rounded-md px-2 py-1 bg-white">
              {Object.entries(IS_MODES).map(([k, v]) => <option key={k} value={k}>{v.label}</option>)}
            </select>
          </div>
        </div>
      </div>
      ...
      <div className="px-5 pb-5 space-y-5">
            ...
            <div key={y} className={`rounded-xl border ${isCur ? 'border-slate-300 ring-2 ring-offset-1 ring-slate-200' : 'border-slate-200'}`}>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-3 p-4">
                {PORTF.map(p => {
                  ...
                  const { base, tax, eff } = computeTax(p, cell.gains, cell.div, inc, model.is_mode)
                  const tipo = p.tax === 'is' ? IS_MODES[model.is_mode].hint : 'progresivo 19-30%'
                  return (
                    <div key={p.key} className="rounded-lg border border-slate-200 p-3 bg-slate-50/40">
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        <span className="text-[10px] text-slate-400">· {p.owner}</span>
                      </div>
                      ...
                      <input
                        ...
                        className="w-full text-[13px] font-mono border border-slate-200 rounded-md px-2 py-1 mb-2 disabled:bg-slate-100 disabled:text-slate-500"
                        placeholder="0" />
        ...
        <div className="text-[10px] text-slate-400 leading-relaxed border-t border-slate-100 pt-3">
          Previsión orientativa, no asesoramiento fiscal. IRPF del ahorro 2026: 19% (0-6k) · 21% (6-50k) · 23% (50-200k) · 27% (200-300k) · 30% (&gt;300k), por persona.
          IS 2026 según régimen de Index (editable arriba). La base son <b>ganancias realizadas</b> del ejercicio; edita el año en curso conforme materialices ventas. {saving && <span className="text-slate-500">· guardando…</span>}
        </div>
      </div>
    </div>
  )
