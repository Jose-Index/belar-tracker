...
export default function CapitalCards({ snapshots, brokers, wallets, onUpdateValues, btcPrice }) {
  ...
  const brokerCards = brokers.map(b => ({
    ...
    color: BROKER_COLORS[b.code] || b.color, editable: true, editKey: b.code,
  }))

  // BTC card: value = qty * live price (or stored usd)
  ...
  const btcCard = {
    ...
    color: BROKER_COLORS.btc, editable: true, editKey: 'btc_qty',
  ...
  const handleEdit = (card) => {
    ...
    setEditValue(card.editIsBtcQty ? String(btcQty) : String(card.value))
  }

  const handleSave = (card) => {
    const val = parseFloat(editValue)
    if (!isNaN(val) && onUpdateValues) {
      onUpdateValues(card.editKey, val)
    }
    setEditing(null)
  }
  ...
  return (
    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
      {cards.map(c => {
        ...
        return (
          <div key={c.code}
            className="bg-white rounded-xl border border-slate-200 p-4 card-hover cursor-pointer"
      ...
      <div className="bg-white rounded-xl border border-slate-200 p-4"
        ...
        <div className="text-xl font-bold font-mono text-sky-600">{formatCurrency(total)}</div>
