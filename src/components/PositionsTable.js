...
export default function PositionsTable({ positions, positionHistory, onRefresh, eurUsdRate = 1.08 }) {
  ...
  const handleClosePosition = async (p) => {
    const reasonInput = prompt(
      `¿Cerrar posición ${p.ticker} (${BROKER_NAMES[p.platform]})?\n\nMOTIVO del cierre:\n  1 = SL (saltó el stop loss en el broker)\n  2 = MANUAL (cierre decidido por José)\n  3 = TP (saltó el take profit)\n\nLa fila desaparece del dashboard pero se conserva su histórico.\nEl ticker pasa al Tablero Rector → RADAR.`,
      '1'
    )
    if (reasonInput === null) return
    const reasonMap = { '1': 'SL', '2': 'MANUAL', '3': 'TP' }
    const closeReason = reasonMap[String(reasonInput).trim()] || 'MANUAL'
    // Fix 21/08/2026: escribir PRIMERO en BD y retirar la fila solo tras confirmar.
    // (El borrado optimista dejaba cierres sin persistir y sin aviso — caso PKOH.)
    const payload = {
      is_open: false,
      close_reason: closeReason,
      closed_date: new Date().toISOString().slice(0, 10),
      closed_value: p.current_value ?? null,
    }
    let { error } = await supabase.from('positions').update(payload).eq('id', p.id)
    if (error) {
      // Tolerante si close_reason aún no existe en el esquema
      const retry = await supabase.from('positions').update({
        is_open: false,
        closed_date: payload.closed_date,
        closed_value: payload.closed_value,
      }).eq('id', p.id)
      error = retry.error
    }
    if (error) {
      console.error('Close failed:', error)
      alert(`⚠️ No se pudo cerrar ${p.ticker}: ${error.message}\nLa posición SIGUE ABIERTA en la base de datos.`)
      onRefresh?.()
      return
    }
