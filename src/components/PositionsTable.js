            ...
            {sorted.map(p => {
              ...
              return (
                <tr key={p.id}>
                  ...
                  <td className={`text-right font-mono text-[7px] ${pnlColor(dailyPct)}`}>
            ...
            {sorted.map(p => {
              ...
              return (
                <tr key={p.id}>
                  ...
                  <td className="text-slate-500 font-mono text-[11px]">
                    {entry.toLocaleDateString('es-ES', entry.getFullYear() === new Date().getFullYear() ? { day: '2-digit', month: 'short' } : { day: '2-digit', month: 'short', year: '2-digit' })}
                  </td>
                  <td className="text-right font-mono text-[12px] text-slate-500">{formatCurrency(invested)}</td>
                  <td className="text-right cursor-pointer" onClick={() => { setEditing(p.id); setEditVal(String(value)) }}>
                    {editing === p.id ? (
