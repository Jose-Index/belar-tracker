...
// Esquema BTP (verificado 19/09 con ?que=esquema): positions = solo abiertas (sin is_open); cierres en position_history.
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const tareas = []
    if (quiere('posiciones')) tareas.push(rest(`positions?select=*&order=broker,ticker&limit=${limite}`).then(d => out.posiciones = d))
    if (quiere('alertas')) tareas.push(rest(`alerts?select=*&order=created_at.desc&limit=${limite}`).then(d => out.alertas = d))
    if (quiere('calendario')) tareas.push(rest(`calendar_events?select=*&event_date=gte.${ahora().slice(0, 10)}&order=event_date&limit=${limite}`).then(d => out.calendario = d))
    if (quiere('liquidez')) tareas.push(rest(`app_state?select=*`).then(d => out.app_state = d))
    if (quiere('historico')) tareas.push(rest(`position_history?select=*&order=closed_date.desc.nullslast,created_at.desc&limit=${limite}`).then(d => out.cerradas = d))
    if (que === 'todo') tareas.push(rest(`weekly_snapshots?select=*&order=week_end.desc&limit=4`).then(d => out.ultimos_snapshots = d))
